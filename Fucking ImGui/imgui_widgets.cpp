...
bool ImGui::SliderScalar_2(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max, ImGui::GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    ImGui::SetCursorPosX(ImGui::CalcItemWidth() + 72);

    const ImRect bb(window->DC.CursorPos + ImVec2(-ImGui::CalcItemWidth() - 50, 0), window->DC.CursorPos + size);
    ...
    else
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetColorU32(col_source));

        ImGui::RenderText(ImVec2(bb_inner.Max.x - 22, bb_inner.Min.y + 0), "A");
